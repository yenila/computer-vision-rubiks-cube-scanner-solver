...
export function Button({ icon, variant = "secondary", children, className = "", type = "button", ...props }) {
    return (_jsxs("button", { type: type, className: `inline-flex h-10 items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold transition duration-200 focus:outline-none focus:ring-2 focus:ring-cyan-300/70 focus:ring-offset-2 focus:ring-offset-slate-950 disabled:cursor-not-allowed ${styles[variant]} ${className}`, ...props, children: [icon, children] }));
}
