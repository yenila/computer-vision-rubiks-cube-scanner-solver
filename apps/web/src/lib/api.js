...
import { parseNotation, validateCubeScan } from "@rubiks/shared";
...
function authUserFromSupabase(user, profile) {
...
async function currentUser() {
    ...
    if (!data.user)
        throw new Error("Sign in before saving scans.");
    return data.user;
}
...
async function sessionFromAuth(session, fallbackUser, name) {
...
function describeInvalidFacelets(facelets) {
    ...
    for (let index = 0; index < cornerFacelets.length; index += 1) {
        ...
        if (!validCorners.has(key))
            invalid.push(`${cornerNames[index]} corner reads ${colors.join("-")}`);
    }
    for (let index = 0; index < edgeFacelets.length; index += 1) {
        ...
        if (!validEdges.has(key))
            invalid.push(`${edgeNames[index]} edge reads ${colors.join("-")}`);
    }
    if (invalid.length)
        return `Check ${invalid.slice(0, 4).join("; ")}.`;
...
function solveFacelets(facelets) {
    ...
    const validPieces = isPermutation(state.center, 6) &&
...
export const api = {
    ...
    solve: async (facelets) => solveFacelets(facelets),
    listScans: async (_token) => {
    ...
    createSolve: async (_token, input) => {
        const user = await currentUser();
        ...
        if (leaderboardReadError)
            throw new Error(leaderboardReadError.message);
        ...
        if (leaderboardWriteError)
            throw new Error(leaderboardWriteError.message);
